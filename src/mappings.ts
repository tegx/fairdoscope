function renderUrl(type: string, value: string): string {
    return `<a href="${value}" target="_blank" rel="noopener noreferrer">${value}</a>`;
}

function renderHdl(type: string, value: any) {

    return fetch(`https://hdl.handle.net/api/handles/${value}`)
        .then(res => {
            if (res.ok) {
                let isType = false;
                let isFairDO = false;
...
function renderText(type: string, value: any) {
    return value as string;
}

function renderObject(type: string, value: any) {
...
export interface DataTypeEntry {
    name: string;
    class: string;
    renderer: (type: string, value: any) => string | Promise<any>
};
